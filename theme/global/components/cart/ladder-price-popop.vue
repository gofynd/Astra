<template>
  <div class="ladder-popup-container" v-if="ladderPrices">
    <div class="ladder-popup-overlay" @click.self="closePopup">
      <div class="ladder-popup">
        ...
        <div class="ladder-popup-price-detail">
          ...
          <div @click="onProductClick" class="ladder-popup-product-title">
            {{ getProductName }}
          </div>
        </div>
...
export default {
  ...
  methods: {
    onProductClick() {
      if (isBrowser && this.item?.product?.slug)
        window.location = `/product/${this.item?.product?.slug}`;
    },
    closePopup(e) {
