<template>
  <div class="brands">
    <div class="brands__breadcrumbs desktop caption-normal">
    ...
    <template v-else-if="context?.brands?.items?.length > 0">
      <h1
        class="brands__title font-header"
      ...
      <div
        class="font-body"
        :class="['brands__description', isMobile ? 'b2' : 'b1']"
      ...
      <div class="brands__cards">
        <fdk-brands-listing :handleinfinite="infiniteFlag">
          <template slot-scope="brandListing">
            <template v-if="infiniteFlag">
              <group-list
                :cardlist="context.brands.items"
                :cardtype="'BRANDS'"
                :show_only_logo="!!getPageConfigValue(page_config, 'logo_only')"
            ...
            <template v-else>
              ...
              >
                <group-list
                  ...
                  :cardtype="'BRANDS'"
                  :show_only_logo="
                    !!getPageConfigValue(page_config, 'logo_only')
                  "
                  :global_config="global_config"
                ></group-list>
              </fdk-infinite-scrolling>
            ...
            <InfiniteLoader
              v-if="context.brands.page.has_next && !infiniteFlag"
            ></InfiniteLoader>
            <div
    ...
    ></fdk-empty-state>
    <div class="brands__breadcrumbs mobile caption-normal font-body">
      <span><fdk-link :link="'/'">Home</fdk-link>&nbsp; / &nbsp;</span>
      <span class="active">Brands</span>
    </div>
  </div>
